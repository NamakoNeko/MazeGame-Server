...
public class GameDropService {
    ...
    public GameDropService(PlayerDao playerDao, ItemDao itemDao, PlayerItemDao playerItemDao, PlayerItemService playerItemService) {
    ...
    @Transactional
    public PlayerItemResult rollDrop(String accountId, DropRequest request) {
        ...
        int position = request.getTargetPosition() == null ? findFirstEmptyPosition(player.getId(), location) : request.getTargetPosition();

        Optional<PlayerItem> existing = playerItemDao.findByPlayerIdAndLocationAndPosition(player.getId(), location, position);
