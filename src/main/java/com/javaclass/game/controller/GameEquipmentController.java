...
public class GameEquipmentController {
    ...
    public ResponseEntity<ApiResponse<?>> equip(
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(gameEquipmentService.equip(accountId, request)));
    ...
    public ResponseEntity<ApiResponse<?>> unequip(
        ...
        try {
            String accountId = jwtUtility.extractPlayerAccountId(jwtUtility.extractToken(authorizationHeader));
            return ResponseEntity.ok(ApiResponse.success(gameEquipmentService.unequip(accountId, request)));
        } catch (IllegalArgumentException exception) {
            return ResponseEntity.badRequest().body(ApiResponse.failure(400, exception.getMessage()));
        }
    }
